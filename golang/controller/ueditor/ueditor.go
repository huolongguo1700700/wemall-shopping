...
func upload(ctx iris.Context) {
	...
	if err != nil {
		utils.Res(ctx, iris.StatusInternalServerError, errResData)
		return
	}
	...
	if index < 0 {
		errResData["state"] = "Invalid file name."
		utils.Res(ctx, iris.StatusInternalServerError, errResData)
		return
	}
	...
	if mimeType == "" {
		errResData["state"] = "Invalid image type."
		utils.Res(ctx, iris.StatusInternalServerError, errResData)
		return
	}
...
func Handler(ctx iris.Context) {
	...
	switch action {
		case "config": {
			utils.Res(ctx, iris.StatusOK, UEditor)
			break
		}
		case "uploadImage": {
			upload(ctx)
