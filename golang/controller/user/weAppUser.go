...
func WeAppLogin(ctx iris.Context) {
	session := config.Sess.Start(ctx)
	SendErrJSON := common.SendErrJSON
	code := ctx.FormValue("code")
	if code == "" {
		SendErrJSON("Code can't be empty.", ctx)
		return
	}
	...
	if err != nil {
		fmt.Println(err.Error(), 222222222)
		SendErrJSON("error", ctx)
		return
	}
	fmt.Println("------------", resp.Body)
	fmt.Println("************", utils.ObjStr(data))
	if _, ok := data["session_key"]; !ok {
		fmt.Println("session_key doen't exist.")
		fmt.Println(data)
		SendErrJSON("error", ctx)
		return
	}
